...
def dns_query(type, name, server):
    ...
    header = struct.pack('!HHHHHH', ID, QR << 15 | OPCODE << 12 | AA << 15 | TC << 15 | RD << 15| RA << 15 | Z << 15 | RCODE, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT)
    ...
    for _ in range(ANCOUNT):
        ...
        type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset+16+2+4+2]) # What is the offset value in bytes? Remember 'H' represent 2 bytes, and 'I' represents one byte, we declared '!HHIH'.
        
        offset += 16 + 2 + 4 + 2# Same value as just calculated
