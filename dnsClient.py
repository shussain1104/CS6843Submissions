...
def dns_query(type, name, server):
    ...
    for _ in range(ANCOUNT):
        ...
        type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset+2+2+4+2]) # What is the offset value in bytes? Remember 'H' represent 2 bytes, and 'I' represents one byte, we declared '!HHIH'.
        
        offset += 2 + 2 + 4 + 2# Same value as just calculated
